...
async def startup_event():
    ...
    try:
        ...
        logger.info(f"Model path: {CONGESTION_MODEL_PATH}")
        ...
        try:
            ...
            logger.warning(f"Could not load stations data at {STATIONS_DATA_PATH}: {str(e)}. Using default coordinates.")
            STATIONS_DF = None
        
        # Load and compute station statistics for lag/derived features
        try:
            train_df = pd.read_csv(CONGESTION_TRAIN_DATA_PATH)
            ...
            logger.warning(f"Could not load training data at {CONGESTION_TRAIN_DATA_PATH}: {str(e)}. Using default values.")
            STATION_STATS = None
