...
async def startup_event():
    ...
    try:
        ...
        logger.info(
            f"Features: {MODEL.n_estimators} trees, max_depth={MODEL.max_depth}"
        )
        ...
        try:
            ...
            logger.warning(
                f"Could not load EVAT.chargers.csv: {str(e)}. Using default coordinates."
            )
            STATIONS_DF = None
        ...
        try:
            ...
            logger.info(
                f"Recent historical data loaded for {len(STATION_RECENT_DATA)} stations"
            )
            ...
            STATION_STATS = (
                train_df.groupby("stationId")[lag_features].mean().to_dict("index")
            )
            ...
            logger.warning(
                f"Could not load train_exogenous_3h.csv: {str(e)}. Using default values."
            )
            STATION_STATS = None
        ...
        logger.info("Starting BE requests")

        logger.info(datetime.now())
        logger.info(os.getenv("NO_THREADS"))

        # Multi-threaded attempt
        def run_async_in_thread(station_id):
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                result = loop.run_until_complete(put_backend(station_id))
                return station_id, result
            finally:
                loop.close()

        station_ids = list(STATIONS_DF["_id"])

        
        with ThreadPoolExecutor(max_workers=int(os.getenv("NO_THREADS"))) as executor:
            futures = {executor.submit(run_async_in_thread, i): i for i in station_ids}

            for future in as_completed(futures):
                station_id, success = future.result()
                #print(station_id)
                if success == False:
                    logger.error(f"Station {station_id} failed to update")
...
def get_station_coordinates(station_id: str) -> tuple:
    ...
    if STATIONS_DF is not None:
        try:
            ...
            logger.warning(
                f"Could not get coordinates for station {station_id}: {str(e)}"
            )
...
def fetch_weather_data(
    target_date: datetime, latitude: float, longitude: float
) -> Dict:
    """
...
def get_lag_features_from_data(station_id: str, target_time: datetime) -> Dict:
    ...
    if STATION_RECENT_DATA is not None and station_id in STATION_RECENT_DATA:
        ...
        logger.debug(
            f"Station {station_id}: lag1={lag_features['arrivals_lag1']:.2f}, "
    ...
    if STATION_STATS is not None:
        ...
        lag_features["arrivals_lag1"] = (
            station_stats.get("arrivals_lag1", 1.5) * variation
        )
        lag_features["arrivals_lag2"] = (
            station_stats.get("arrivals_lag2", 1.5) * variation
        )
        lag_features["arrivals_lag4"] = (
            station_stats.get("arrivals_lag4", 1.5) * variation
        )
        lag_features["arrivals_ma4"] = (
            station_stats.get("arrivals_ma4", 1.5) * variation
        )
        lag_features["arrivals_ma8"] = (
            station_stats.get("arrivals_ma8", 1.5) * variation
        )
        lag_features["arrivals_pct_change"] = station_stats.get(
            "arrivals_pct_change", 0.0
        )
        lag_features["arrivals_diff"] = station_stats.get("arrivals_diff", 0.0)
        lag_features["arrivals_ewma_4"] = (
            station_stats.get("arrivals_ewma_4", 1.5) * variation
        )
    else:
...
def fetch_weather_data_cached(target_time, latitude, longitude):
    # Round coordinates to be less precise
    lat_rounded = round(latitude, 2)
    lon_rounded = round(longitude, 2)
    
    cache_key = (target_time, lat_rounded, lon_rounded)
    
    if cache_key in weather_cache:
        return weather_cache[cache_key]
    
    # Fetch from API
    weather = fetch_weather_data(target_time, lat_rounded, lon_rounded)
...
def engineer_features(station_id: str, target_time: datetime) -> pd.DataFrame:
    ...
    weather = fetch_weather_data_cached(target_time, latitude, longitude)
    ...
    features["temp_x_precipitation"] = (
        features["temp_avg_c"] * features["precipitation_mm"]
    )
...
async def predict_single(request: PredictionRequest):
    ...
    try:
        ...
        logger.info(
            f"Station {request.station_id}: congestion_level = {congestion_level}"
        )

        # Build response
        response = PredictionResponse(
            station_id=request.station_id, congestion_level=congestion_level
        )
        return response
...
async def predict_batch(request: BatchPredictionRequest):
    ...
    try:
        ...
        logger.info(
            f"Batch predicting for {len(request.station_ids)} stations at {target_time}"
        )
        ...
        for station_id in request.station_ids:
            ...
            pred_response = PredictionResponse(
                station_id=station_id, congestion_level=congestion_level
            )
        ...
        return BatchPredictionResponse(
            predictions=predictions, count=len(predictions), timestamp=target_time
        )
...
async def put_backend(station_id: str):
    """
    Predict congestion for a stations and sends it to EVAT backend server

    Args:
        station_id: String of station id to predict and send
    ...
    try:
        ...
        # Send to server
        headers = {"Authorization": f"Bearer {os.getenv("ADMIN_AUTH")}"}
        res = requests.put(
            os.getenv("BE_SERVER") + "/api/predict/congestion",
            params={"id": station_id, "level": congestion_level},
            headers=headers,
        )
        if res.status_code == 201:
            ...
            print(res.json())
            logger.error("Backend did not return 201")
            return False
    except Exception as e:
        logger.error(f"Prediction error: {str(e)}")


async def post_backend(station_ids: List):
    """
    Predict congestion for a stations and sends it to EVAT backend server

    Args:
        station_ids: List of station ids to predict and send
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {os.getenv("ADMIN_AUTH")}"}
        res = requests.post(
            os.getenv("BE_SERVER") + "/api/predict/congestion/batch",
